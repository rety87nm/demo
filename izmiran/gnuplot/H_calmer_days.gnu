...
set title "Вариации по магнитоспокойным дням ".current_date." "  font ",14"
set multiplot
plot "<psql -h localhost -d izmiran -U izmiran -A -t -F \" \" -c \"select date_utc::time, h from mag_variations_spg where date_utc::date = CURRENT_DATE and measurement_is_correct(x,y,z) order by date_utc::time\"" using 1:2 with lines lt 7 ti "Значение H в данный момент", "<psql -h localhost -d izmiran -U izmiran -A -t -F \" \" -c \"select * from sr_variations order by time\"" using 1:2 with lines lt 6 ti "Значение H магнитоспокойных дней"
unset multiplot
